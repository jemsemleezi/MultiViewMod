    ...
    public class SecondaryCameraWindow : Window
    {
        ...
        public override void PreOpen()
        {
            ...
            if (savedRect.HasValue)
            {
                ...
                // Log.Message($"[MultiViewMod] 恢复窗口位置: {this.windowRect}");
            }
            else
            {
                ...
                // Log.Message($"[MultiViewMod] 使用默认窗口位置: {this.windowRect}");
            }
            ...
            if (savedZoom.HasValue && cameraController != null)
            {
                ...
                // Log.Message($"[MultiViewMod] 恢复缩放比例: {savedZoom.Value}");
            }
        }
        ...
        public override void PostClose()
        {
            ...
            // Log.Message($"[MultiViewMod] 窗口关闭，位置和缩放比例已保存");
        }
        ...
        private void DrawObservationView(Rect inRect)
        {
            try
            {
                ...
                {
                    GUI.DrawTexture(inRect, renderTexture, ScaleMode.ScaleToFit);
                }
                else
                {
        ...
        private void DrawOverlayControls(Rect inRect)
        {
            try
            {
                ...
                Texture2D pinIcon = IsPinned ? TexButton.LockNorthUp : TexButton.Play; // 使用指北图标表示固定状态
                if (Widgets.ButtonImage(pinRect, pinIcon))
                {
                    ToggleWindowPin();
                }
                string pinTooltip = IsPinned ? "MultiViewMod_UnpinWindowTooltip".Translate() : "MultiViewMod_PinWindowTooltip".Translate();
        ...
        private void ToggleWindowPin()
        {
            ...
            if (IsPinned)
            {
                ...
                draggable = false;
                resizeable = false;
                Log.Message("[MultiViewMod] 窗口已固定，快捷键关闭已禁用");
            }
            else
            {
                // 取消固定时：恢复拖动和调整大小
                draggable = true;
                resizeable = true;
                Log.Message("[MultiViewMod] 窗口已取消固定，快捷键关闭已启用");
            }
        }
